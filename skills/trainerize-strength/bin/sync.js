...
import { CACHE_DIR, WORKOUT_DIR, ensureDirs, saveConfig } from './config.js';
...
export async function sync(client, cfg, opts = {}) {
    const log = opts.log ?? (() => { });
    ensureDirs();
    ...
    for (let i = 0; i < todo.length; i += BATCH) {
        ...
        for (const w of res.dailyWorkouts ?? []) {
            if (!Number.isSafeInteger(w?.id) || w.id <= 0)
                continue; // ids become filenames
