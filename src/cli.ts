...
function ask(question: string, hidden = false): Promise<string> {
  return new Promise(resolve => {
    ...
    if (hidden) {
      ...
      (rl as any)._writeToOutput = (s: string) => { if (s.includes(question)) process.stdout.write(s); };
    }
...
const parseSubdomain = (s: string) => s.replace(/^https?:\/\//, '').split('/')[0].replace(/\.trainerize\.com$/i, '');
...
function cmdExport() {
  ...
  if (values.out) { writeFileSync(values.out, body); err(`✓ ${rows.length} sets → ${values.out}`); }
  else process.stdout.write(body);
}

function cmdStatus() {
  const cfg = loadConfig();
  if (!cfg) return err('Not logged in. Run `trainerize login`.');
...
async function cmdServe() {
  ...
  if (!values['no-open']) execFile(process.platform === 'darwin' ? 'open' : 'xdg-open', [url], () => {});
}

function cmdLogout() {
  const cfg = loadConfig();
  if (cfg) deletePassword(cfg.email);
  clearToken();
  err('✓ Stored password and session removed. Cached workouts remain in ' + HOME);
}

const commands: Record<string, () => unknown> = { login: cmdLogin, sync: cmdSync, stats: cmdStats, export: cmdExport, status: cmdStatus, serve: cmdServe, logout: cmdLogout };
const cmd = positionals[0];
if (!cmd || values.help || !commands[cmd]) { console.log(HELP); process.exit(cmd && !values.help ? 1 : 0); }
Promise.resolve(commands[cmd]()).catch(e => { err('✗ ' + (e as Error).message); process.exit(1); });
